...
function getToken() {
  try {
    ...
    console.warn("AGS Panel: Token retrieval failed", e);
    return "";
  }
}
...
async function loadConfig() {
  try {
    console.log('%cAGS Panel: Loading config via REST', 'color:#63b3ed', _entryId);
    const resp = await fetch(`/api/advanced_group_sensor/config?entry_id=${_entryId}`, {
    ...
    console.error("AGS Panel: config load error:", e);
...
async function loadStates() {
  ...
  _statesLoading = true;
  try {
    const resp = await fetch(`/api/advanced_group_sensor/states`, {
      headers: { "Authorization": `Bearer ${getToken()}` }
    });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const result = await resp.json();
    ...
    console.error("AGS Panel: states load error:", e);
    _states = {};
    _allEntityList = [];
  }
...
function render() {
  ...
  if (!app) return;
  console.log('%cAGS Panel: render() called', 'color:#63b3ed');
...
<div style="min-height:100vh;background:#080a0f;display:flex;align-items:flex-start;justify-content:center;padding:24px;font-family:'DM Sans','Segoe UI',sans-serif;color:#e2e8f0;box-sizing:border-box;">
  <div style="width:560px;max-width:100%;background:#0f1219;border-radius:16px;box-shadow:0 32px 80px rgba(0,0,0,0.6),0 0 0 1px rgba(255,255,255,0.05);overflow:visible;display:flex;flex-direction:column;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px 0;">
      <span style="font-size:0.72rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#94a3b8;">⚙ ADVANCED GROUP SENSOR — CONFIGURATION</span>
...
function buildGeneral() {
  ...
  return `
    ${buildCard("Sensor Identity", `
      ${buildField("Your Sensor Name", `<input id="f-name" type="text" value="${esc(c.name || "")}" placeholder="e.g. advanced_group_sensor_1" style="${inputStyle()}">
...
console.log('%cAdvanced Group Sensor v8.2.0 — Vanilla JS panel initializing', 'color:#39FF14; font-weight:bold');
