/**
 * Advanced Group Sensor Panel v8.2.0
...
try {
  ...
  console.error("AGS Panel: Lit detection failed", e);
  LitElement = HTMLElement;
...
try {
  console.log('%cAdvanced Group Sensor v8.2.0 → Starting definePanel()', 'color:#39FF14; font-weight:bold');
  definePanel();
  console.log('%cAdvanced Group Sensor v8.2.0 → Successfully registered', 'color:#39FF14; font-weight:bold');
} catch (e) {
...
// Main panel element
// ---------------------------------------------------------------------------
class AdvancedGroupSensorPanel extends LitElement {
  ...
  connectedCallback() {
    ...
    console.log('%cAGS Panel: deferred init via requestAnimationFrame', 'color:#63b3ed');
  }

  _maybeLoadConfig() {
    if (this._config || this._loading || !this._hass || !this._entryId) return;
    this._loading = true;
    console.log('%cAGS Panel: Loading config for entry', 'color:#63b3ed', this._entryId);
    this._loadConfig();
  }
  ...
  async _loadConfig() {
    try {
      console.log('%cAGS Panel: Sending get_config WS call', 'color:#63b3ed', this._entryId);
      const result = await Promise.race([
        this.hass.callWS({
          type: "advanced_group_sensor/get_config",
          entry_id: this._entryId,
        }),
        new Promise((_, reject) => setTimeout(() => reject(new Error("WebSocket timeout after 8s")), 8000))
      ]);
      console.log('%cAGS Panel: Config loaded successfully', 'color:#39FF14');
  ...
  async _loadStates() {
    ...
    try {
      const result = await this.hass.callWS({
        type: "advanced_group_sensor/get_states",
        entry_id: this._entryId,
      });
      ...
      console.log("AGS Panel: error loading states:", e);
      this._states = {};
      this._allEntityList = [];
    }
    this.requestUpdate();
  ...
  async _saveConfig() {
    ...
    try {
      ...
      await this.hass.callWS({
        type: "advanced_group_sensor/save_config",
        entry_id: this._entryId,
      ...
      <div class="panel-root">
        <div class="dialog">
          <div class="dialog-header">
            <span class="dialog-title">⚙ ADVANCED GROUP SENSOR — CONFIGURATION</span>
          ...
          <div class="field">
            ...
            <div style="display:flex;gap:8px;align-items:center">
              <input type="text" .value="${c.name || ""}"
                ...
                placeholder="e.g. advanced_group_sensor_1"
                @input="${e => {
          ...
          ${this._showRenameWarning ? html`
            <div style="background:rgba(246,173,85,0.08);border:1px solid rgba(246,173,85,0.4);border-radius:10px;padding:16px;display:flex;flex-direction:column;gap:12px;">
              ...
              <div style="font-size:0.85rem;color:#94a3b8;line-height:1.6">
                ...
                After saving, go to <strong style="color:#e2e8f0">Settings → Devices &amp; Services → Advanced Group Sensor</strong>, find this device, click the pencil icon, and update the entity ID to match. Then reload the integration from the same page using the three-dot menu → Reload.
              </div>
...
if (!customElements.get("advanced-group-sensor-panel")) {
  customElements.define("advanced-group-sensor-panel", AdvancedGroupSensorPanel);
}
} // end definePanel
